...
def check_dir(file_route, flag=False):
    """
    检查路径是否存在，不存在会直接创建
    :param file_route: 路径
    :param flag: 是否删除已存在的路径，会直接清空里面的文件
    :return:
    """
    if os.path.splitext(file_route)[1]:
        file_name = os.path.basename(file_route)
        file_route = file_route.rsplit(file_name, 1)[0]
    try:
        if flag and os.path.exists(file_route):
            shutil.rmtree(file_route)
